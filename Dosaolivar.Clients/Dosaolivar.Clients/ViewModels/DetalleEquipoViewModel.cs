...
namespace SmartHotel.Clients.Core.ViewModels
{
    ...
    public class DetalleEquipoViewModel : ViewModelBase
    {
        ...
        {
            Console.WriteLine("DetalleEquipoViewModel ASYNC");
            ...
            if (navigationData != null)
            {
                ...
                equipo = navigationData as EquipoModel;
                Console.WriteLine("Se carga Detalle EquipoViewModel con datos: " + Equipo.boquilla1id);
                // Asignamos el usuario
                Equipo.username = AppSettings.sessionUsername;
                // Cargamos la lista de boquillas por si estamos volviendo de insertar una antes de insertar el equipo en la bbdd
                getListaBoquillasDiscos();
                if (Equipo.fabricante == null || Equipo.fabricante == "null")
                {
                    Equipo.fabricante = "";
                }
                if (equipo.fabricante == null || equipo.fabricante == "null")
                ...
                if (equipo.nbastidor == null || equipo.nbastidor == "null")
                {
                    equipo.nbastidor = "";
                }
        ...
        async Task NuevoEquipoAsync()
        {
            ...
            Console.WriteLine("para insertar");
            if (equipo.capacidad == "" || equipo.capacidad == null)
            {
                validation = false;
                await Application.Current.MainPage.DisplayAlert("Atención", "Debe de introducir la capacidad del equipo", "OK");
            }
            if (equipo.nombre == "" || equipo.nombre == null)
            {
                validation = false;
                await Application.Current.MainPage.DisplayAlert("Atención", "Debe de introducir el nombre del equipo", "OK");
            }
            if (equipo.boquilla1id == null)
            {
                validation = false;
        ...
        // Seleccionamos boquilla
        public ICommand DeleteEquipoCommand => new AsyncCommand(DeleteEquipoAsync);
        public ICommand UpdateEquipoCommand => new AsyncCommand(UpdateEquipoAsync);
        async Task DeleteEquipoAsync()
        {
            MessagingCenter.Send(this, MessengerKeys.ActualizacionListadoParcelasSolicitado);
            var answerDelete = await Application.Current.MainPage.DisplayAlert("Borrar equipo", "¿Esta seguro que desea eliminar el equipo?", "Si", "No");
            if (answerDelete)
            {
                Console.WriteLine("vamos a borrar equipo con id " + Equipo.id);
                var answer = await CustomRequest.DeleteEquipoAPIModel(Equipo.id);
                if (answer.success)
                {
                    //await NavigationService.NavigateBackAsync();
                    await Application.Current.MainPage.DisplayAlert("Atención", "Equipo eliminado con exito", "Ok");
                    await NavigationService.NavigateToAsync<MainViewModel>();
                }
                else
                {
                    await Application.Current.MainPage.DisplayAlert("Atención", "Ocurrió un error al borrar el equipo, inténtelo mas tarde", "Ok");
                    await NavigationService.RemoveLastFromBackStackAsync();
                    await NavigationService.NavigateToAsync<MainViewModel>();
                    //await NavigationService.NavigateBackAsync();
                }
        ...
        async Task UpdateEquipoAsync()
        {

            await Application.Current.MainPage.DisplayAlert("Atencion", "Función disponible en proximas versiones", "Ok");
        }
